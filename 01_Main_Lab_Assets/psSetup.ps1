...
$openAIModel1 = az cognitiveservices account deployment create -g $rgName -n $openAIResource --deployment-name "gpt-4" --model-name "gpt-4" --model-version "0613" --model-format OpenAI --sku-capacity 120 --sku-name "Standard" 
$openAIModel2 = az cognitiveservices account deployment create -g $rgName -n $openAIResource2 --deployment-name "gpt-4o" --model-name "gpt-4o" --model-version "2024-05-13" --model-format OpenAI --sku-capacity 120 --sku-name "Standard"
$openAIModel3 = az cognitiveservices account deployment create -g $rgName -n $openAIResource2 --deployment-name "text-embedding-ada-002" --model-name "text-embedding-ada-002" --model-version "2" --model-format OpenAI --sku-capacity 120 --sku-name "Standard"

#retirieving primary key
$openAIPrimaryKey = az cognitiveservices account keys list -n $openAIResource -g $rgName | ConvertFrom-Json
$openAIPrimaryKey = $openAIPrimaryKey.key1

#retrieving openai endpoint
$openAIEndpoint2 = az cognitiveservices account show -n $openAIResource2 -g $rgName | ConvertFrom-Json
$openAIEndpoint2 = $openAIEndpoint2.properties.endpoint

#retirieving primary key
$openAIPrimaryKey2 = az cognitiveservices account keys list -n $openAIResource2 -g $rgName | ConvertFrom-Json
$openAIPrimaryKey2 = $openAIPrimaryKey2.Key1
...
$contentsafetykey = az cognitiveservices account keys list -n $contentsafety -g $rgName | ConvertFrom-Json
$contentsafetykey = $contentsafetykey.key1
...
$deployment = Invoke-RestMethod -Uri "https://$sites_adx_thermostat_realtime_name.scm.azurewebsites.net/api/publish?type=zip" -Method Post -Headers @{Authorization = "Bearer $TOKEN_2"} -InFile "./artifacts/binaries/app-adx-thermostat-realtime.zip" -ContentType "application/zip"
...
$TOKEN_3 = (az account get-access-token --query accessToken -o tsv) -replace '"', ''

$deployment = Invoke-RestMethod -Uri "https://$func_shopping_copilot.scm.azurewebsites.net/api/zipdeploy" -Method Post -Headers @{Authorization = "Bearer $TOKEN_3"} -InFile "./artifacts/binaries/func-shopping-copilot.zip" -ContentType "application/zip"
...
$TOKEN_4 = (az account get-access-token --query accessToken -o tsv) -replace '"', ''

$deployment = Invoke-RestMethod -Uri "https://$shoppingWebappname.scm.azurewebsites.net/api/publish?type=zip" -Method Post -Headers @{Authorization = "Bearer $TOKEN_3"} -InFile "./webapp-shopping-copilot.zip" -ContentType "application/zip"

Write-Host "-----------------Shopping Copilot webapp build deployment compeleted ---------------"

# Invoke Webapp from powershell
$url = "https://"+ $sites_adx_thermostat_realtime_name + ".azurewebsites.net/"
